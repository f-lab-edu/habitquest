...
public class HabitQuestExceptionHandler {
  ...
  public ResponseEntity<CommonResponse<?>> exceptionHandler(HabitQuestException e) {
    ...
    return ResponseEntity.status(errorType.getHttpStatus()).body(new CommonResponse<>(errorType.getCode(), errorType.getHttpStatus().value(), errorType.getMessage(), null));
  }
}